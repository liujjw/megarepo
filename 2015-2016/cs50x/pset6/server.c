...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // valid string syntax to check header request line grammar
            const char* GET = "GET ";
            const char* HTTP = "HTTP/1.1";
            
            // ptrs for strstr function returning another ptr
            const char* f_slash = "/";
            const char* question = "?";
            
            // check method GET and a space  
            ...
            // ptr to first slash in request_target
            char* request_target = strstr(line, f_slash);
            // buffer to hold request target, 4096 chars limit
            char request_buffer[LimitRequestFieldSize];
            // copy cotents of request target ptr (contents now include only [request-target SP HTTP-version CRLF]) to request buffer
            strcpy(request_buffer, request_target);
            // look for http 1.1
            char* is_http = strstr(request_buffer, HTTP);
            // if there is a http, go back a space and add a nul terminator to request buffer, which is now just the request file
            if(is_http != NULL && /*SP before http?*/ *(is_http - 1) == ' ')
                *(is_http - 1) = '\0';
            else
                return error(505);
            /* request_buffer is also file for concatenating to the root*/

            // check if request buffer has a period for file extension and no spaces
            if(strchr(request_buffer, ' ') != NULL)
                return error(400);
            if(strchr(request_buffer, '.') == NULL)
                return error(501);
            if(strchr(request_buffer, '\"') != NULL)
            ...
            // declaring a query, first setting it to be nulled before if theres no question mark
            char query[LimitRequestFieldSize];
            query[0] = '\0';

            // serach for a question mark, then proceed if one is found
            char* option = strstr(request_buffer, question);
            if(option != NULL)
            {
                // if question mark is followed by nothing(nul), then set qeury to nul
                if(*(option + 1) == '\0')
                    query[0] = '\0';    
                
                // if theres a .xyz file extension, clunky checking of no question mark in absolute path***************************
                else
                {
                    // copy over query from ptr after question mark
                    // copying from request_buffer, starting from question mark, to query until nul 
                    int i;
                    for(i = 0; *(option + i + 1) != '\0'; i++)
                    {
                    ...
                    // error if theres a quote(s) in query
            ...
            // completely oblivious to strcat fucntion at time of writing
            // absolute path
            char path[LimitRequestFieldSize];
            // if there was a question mark
            if(option != NULL)
            {
                // nul terminate question mark ptr location in request buffer
                *option = '\0';    
            }
            // copy over the root to path and look for nul terminator, point which we concatenate
            strcpy(path, root);
            char* root_nul = strchr(path, '\0');
            
            // iterate over request_buffer until nul terminator is reached
            int k;
            for(k = 0; *(request_buffer + k) != '\0'; k++)
            {
                // copy over each char in request_buffer to the start of the end of path(root nul)
                *(root_nul + k) = *(request_buffer + k);
            } 
            *(root_nul + k) = '\0';
            ...
            char extension[10];
            char* dot_xyz = strchr(path, '.');
            int j;
            for(j = 0; *(dot_xyz + j + 1) != '\0'; j++)
            {    
                extension[j] = *(dot_xyz + j + 1);
            }
            extension[j] = '\0';
...
const char* lookup(const char* extension)
{
    ...
    // file extension string ptrs and returns
    const char* css = "css"; 
    ...
    const char* ico = "ico"; 
        const char* image_x_icon = "image/x-icon";
    const char* jpg = "jpg"; 
    ...
    // returing headers if extension matches 
