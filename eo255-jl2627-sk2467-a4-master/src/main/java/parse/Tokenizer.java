...
public class Tokenizer implements Iterator<Token> {
	...
	private boolean isComment = false; // Represents whether we are parsing inside a comment
	 ...
	 *
	 * Effect: updates nextChar to the proper character and isComment to the proper
	 * value
	 *
	...
	private void lexOneToken() throws IOException {
		...
		// consume whitespace or comment
		while (isComment || Character.isWhitespace(c)) {
			if (c == '\n') {
				lineNumber++;
				isComment = false;
			} else if (c == LookAheadBuffer.EOF) {
				addEOFToken();
				return;
			}
			c = in.next();
	...
	/**
	 * Lexes a slash character '/'. If the slash is followed by another slash '/',
	 * then it is part of a comment, and should be ignored. Otherwise it represents
	 * a division sign.
	 * 
	 * Effect: sets field {@code isComment} to true if it is a comment
	 * 
	 * @throws IOException if an IOException was thrown when trying to read from the
	 *                     source Reader
	 */
	private void lexSlash() throws IOException {
		if (in.peek() == '/') {
			isComment = true;
			// "Skip over" the comment and consume the next non-comment character
			lexOneToken();
		} else
	...
	/**
	 * Lexes a hyphen character '-'. If the hyphen is followed by "->", then it is
	 * part of an arrow "-->". Otherwise it represents a minus sign.
	 *
	 * <p>
